module.exports = (req, res) => {
  ...
  db.run([
    cb => {
      const query = 'SELECT * FROM comment WHERE post_id = $1 AND user_id = $2;';
    ...
    cb => {
      ...
      db.query(query, values, (err, result) => {
        if(err)return cb(err);
        cb(null, { my_comment: result.rows[0] });
      });
    },
    (pre, cb) => {
    ...
    (pre, cb) => {
      const query = `
        SELECT id, type, text 
        FROM comment 
        WHERE post_id = $1 AND user_id != $2 AND text != $3
